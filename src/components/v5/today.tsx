  ...
  return (
    <div className="relative h-36 w-36 shrink-0">
      <svg viewBox="0 0 120 70" className="absolute -top-2 left-1/2 h-12 w-24 -translate-x-1/2 opacity-70" aria-hidden>
        {[28, 50, 72].map((x, i) => <path key={x} className="v5t-steam" style={{ animationDelay: `${i * 0.5}s` }} d={`M${x},64 q-10,-16 0,-30 q10,-14 0,-28`} fill="none" stroke="#9fd8cf" strokeWidth="3.5" strokeLinecap="round" />)}
      </svg>
      <svg viewBox="0 0 160 150" className="absolute inset-0 h-full w-full drop-shadow-[0_18px_30px_rgba(40,20,8,0.45)]">
        ...
        <g clipPath="url(#v5tclip)">
          ...
          <path className="v5t-wave" d="M22,72 q12,-9 24,0 t24,0 t24,0 t24,0 t24,0" fill="none" stroke="#fff7ec" strokeWidth="3" strokeLinecap="round" opacity="0.85" style={{ transform: `translateY(${(1 - pct) * 60}px)`, transition: "transform 600ms cubic-bezier(0.16,1,0.3,1)" }} />
        </g>
...
export function V5Today() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="v5today relative mx-auto max-w-[1080px] space-y-6 font-[family-name:var(--font-inter)] text-[#3a2415]">
      <style>{`
        ...
        .v5t-steam{animation:v5tsteam 3.4s ease-in-out infinite}
        @keyframes v5twave{0%{transform:translateX(0)}100%{transform:translateX(-48px)}}
        .v5t-wave{animation:v5twave 2.6s linear infinite}
        @keyframes v5tfloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-7px)}}
        .v5t-float{animation:v5tfloat 6s ease-in-out infinite}
        .v5t-bean{position:absolute;border-radius:50%;background:radial-gradient(circle at 35% 30%,#6f4a2a,#3a2412);opacity:.06;pointer-events:none}
        .v5t-bean::after{content:"";position:absolute;inset:0;border-radius:50%;background:linear-gradient(transparent 47%,rgba(0,0,0,.35) 48% 52%,transparent 53%);transform:rotate(28deg)}
        @media (prefers-reduced-motion: reduce){.v5today *{animation:none !important}}
      `}</style>

      <span className="v5t-bean v5t-float" style={{ width: 54, height: 70, top: 120, right: 30, transform: "rotate(20deg)" }} />
      <span className="v5t-bean v5t-float" style={{ width: 34, height: 44, bottom: 80, left: 24, animationDelay: "1.5s" }} />
        ...
        <div className="relative flex flex-col items-center justify-between gap-6 p-7 md:flex-row">
          <div className="max-w-md">
            ...
            <div className="mt-5 flex flex-wrap gap-2.5">
              ...
              <Link href="/dashboard-v5" className="inline-flex items-center gap-1.5 rounded-full border border-white/25 px-4 py-2 text-sm font-medium text-[#f7efe2] hover:bg-white/10">Back to dashboard <ArrowUpRight className="size-4" /></Link>
            </div>
          </div>
