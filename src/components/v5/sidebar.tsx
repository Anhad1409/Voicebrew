...
import { PanelLeftClose, PanelLeftOpen, Search, ChevronDown } from "lucide-react";
...
export function V5Sidebar() {
  ...
  const isActive = (href: string) =>
    pathname === href || (href !== "/dashboard-v2" && href !== "/dashboard-v5" && pathname.startsWith(href));

  return (
    <aside
      data-tour="nav"
      className={cn(
        "z-20 flex h-full shrink-0 flex-col border-r border-sidebar-border bg-porcelain transition-[width] duration-300 ease-out",
        ...
        {navGroups.map((group) => {
          ...
          <div key={group.key} className="flex flex-col gap-0.5">
            ...
            {groupOpen && nav.filter((n) => n.group === group.key).map((item) => {
              const href = item.href === "/dashboard-v2" ? "/dashboard-v5" : item.href === "/today" ? "/dashboard-v5/today" : item.href;
