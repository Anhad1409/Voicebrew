...
// Lightweight spotlight walkthrough. Auto-runs once per storageKey; replays on
// window "start-tour" event. No dependencies.
export function Tour({ steps, storageKey }: { steps: TourStep[]; storageKey: string }) {
  ...
  useEffect(() => {
    ...
    if (!seen) t = setTimeout(() => setIdx(0), 700);
    const onStart = () => setIdx(0);
    window.addEventListener("start-tour", onStart);
    return () => { clearTimeout(t); window.removeEventListener("start-tour", onStart); };
  }, [storageKey]);
  ...
  useEffect(() => {
    ...
    update();
    const id = setTimeout(update, 320); // settle after scroll
  ...
  const finish = () => { localStorage.setItem(storageKey, "1"); setIdx(null); };
  const pad = 8;
